...
from embodied.envs.leo_ros2_env import LeoROS2Env
from embodied.envs import from_gym

# from leo_ros2_env import LeoROS2Env
...
def main(argv=None):
    ...
    for name in parsed.configs:
        # Ensure env.leoros exists before merging
        if "env" in configs[name] and "leoros" in configs[name]["env"]:
            if "env" not in config:
                config.env = {}
            if "leoros" not in config.env:
                config.env["leoros"] = {}

        # Manually merge nested dictionary
        config.env["leoros"].update(configs[name]["env"]["leoros"])

        print("Merged config.env:", config.env)  # Debug print
...
def make_logger(config):
    ...
    outputs = []
    outputs.append(elements.logger.TerminalOutput(
        config.logger.filter, 'Agent'))
    for output in config.logger.outputs:
        if output == 'jsonl':
            ...
            proj = 'embodied' if logdir.startswith(
                ('/cns/', 'gs://')) else 'debug'
...
def make_env(config, index, **overrides):
    ...
    ctor = {
        ...
        'leoros': lambda task, **kw: from_gym.FromGym(
            LeoROS2Env(**kw), **kw),
