...
class LeoRoverEnv(gym.Env):
    ...
    def __init__(self, 
                 ...
                 goal_x_range=(-10.0, 10.0),
                 goal_y_range=(-10.0, 10.0),
                 init_x_range=(-10.0, 10.0),
                 init_y_range=(-10.0, 10.0),
                 max_linear_velocity=0.5,
                 max_angular_velocity=1.0,
                 connection_timeout=10,  # Reduced timeout
                 require_imu=False,      # Make IMU optional
        ...
        self.require_imu = require_imu   # Whether IMU is required
        ...
        self.use_odom_for_orientation = not require_imu
        ...
        # Only subscribe to IMU if required
        if require_imu:
            ...
            # If IMU is not required, mark it as received
            self.received_imu = True
            print("IMU data not required, using odometry for orientation")
    ...
    def _is_robot_flipped(self):
        ...
        # Only check roll and pitch if we're using IMU data
        if not self.use_odom_for_orientation:
            return abs(self.current_roll) > FLIP_THRESHOLD or abs(self.current_pitch) > FLIP_THRESHOLD
        return False  # Can't detect flips with odometry alone
    ...
    def _spin_until_sensor_data(self, timeout=1.0):
        ...
        required_sensors = [self.received_image, self.received_odom]
        if not self.use_odom_for_orientation:
            required_sensors.append(self.received_imu)
            
        while not all(required_sensors):
            rclpy.spin_once(self.node, timeout_sec=0.01)
            # Update required_sensors list
            required_sensors = [self.received_image, self.received_odom]
            if not self.use_odom_for_orientation:
                required_sensors.append(self.received_imu)
    ...
    def _check_connections(self, timeout):
        ...
        start_time = time.time()
        while not self.received_image or not self.received_odom or (self.require_imu and not self.received_imu):
            ...
            if time.time() - start_time > timeout:
                ...
                if self.require_imu and not self.received_imu:
                    missing.append("IMU")
    ...
    def odom_callback(self, msg):
        ...
        try:
            ...
            # If we're using odometry for orientation, extract the yaw from the quaternion
            if self.use_odom_for_orientation:
                self.current_roll, self.current_pitch, self.current_yaw = quat2euler([
